...
import 'package:streambox_core/src/strategies/impl/cache_then_refresh_strategy.dart';
...
void main() {
  group('ChainedSourcesRepo integration tests (CacheThenRefreshStrategy)', () {
    ...
    test(
      ...
      () async {
        ...
        final result = await repo.fetchAwait(
          const _Params('req-id-001', 'test-brand'),
        );
    ...
    test(
      'returns ProductModel list when both fetches succeed (response)',
      () async {
        final emitted = <DataState<List<_ProductModel>>>[];
        final sub = repo.stream.listen(emitted.add);
        _primaryFetchThrow = false;
        ...
        expect(emitted[2], isA<DataSuccess<List<_ProductModel>>>());
        expect(
          (emitted[2] as DataSuccess<List<_ProductModel>>).value,
          isEmpty,
        );
...
class _FakeRetrofitBackendProductsApi {
  Future<_BackendResponse> fetchProducts({
    ...
    _differentResponse ? const _BackendResponse([]) : _backendResponse,
  );
}
