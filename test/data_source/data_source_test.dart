...
void main() {
  group('BaseDataSource', () {
    late DataSource<_Params, String> dataSource;

    setUp(() {
      dataSource = _MockDataSource(
        cacheStrategy: _MockStrategy(),
      );
    });
    ...
    test('emits DataSuccess on successful fetch', () async {
      final events = <RequestPayload<_Params, String>>[];
      final sub = dataSource.stream.listen(events.add);

      dataSource.fetch(const _Params(2));

      await Future<void>.delayed(Duration.zero);
      expect(events.last, isA<RequestPayload<_Params, String>>());
    ...
    test('emits DataError on fetch exception', () async {
      ...
      dataSource.fetch(const _Params(404));

      await Future<void>.delayed(Duration.zero);
      expect(events.last, isA<RequestError<_Params, String>>());
    ...
    test('emits DataInitial after flush', () async {
      ...
      await Future<void>.delayed(Duration.zero);
      expect(events.first, isA<RequestPayload<_Params, String>>());
      expect(events.length, equals(2));
      expect(
        (events.first as RequestSuccess<_Params, String>).value,
        equals('something: 2'),
      );
      expect(events.last, isA<RequestInitial<_Params, String>>());
    ...
    test('emits updated data on second fetch if strategy allows', () async {
      ...
      expect(
        events.whereType<RequestPayload<_Params, String>>().length,
        greaterThan(1),
      );
      expect(events.last, isA<RequestPayload<_Params, String>>());
...
class _MyApi {
  ...
  Future<String> fetchSomething(_Params? params) async {
...
class _MockDataSource extends BaseDataSource<_Params, String> {
  _MockDataSource({required super.cacheStrategy});

  final api = _MyApi();

  @override
  Future<String> request(_Params? params) {
...
class _MockStrategy extends CacheThenRefreshStrategy<_Params, String> {
  _MockStrategy() : super(cache: _MockMemoryCache());

  @override
  String resolveKey(_Params? params) => '${params?.id ?? 0}';
}
